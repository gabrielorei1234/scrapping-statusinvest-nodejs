...
async function scrapeStatusInvest() {
  ...
  await page.waitForSelector(".select-dropdown.dropdown-trigger", {
    visible: true,
  });
  ...
  await page.waitForSelector(".table-scroll table tbody tr.item", {
    timeout: 10000,
  });
  ...
  const data = await page.evaluate(() => {
    const rows = Array.from(
      document.querySelectorAll(".table-scroll table tbody tr.item")
    );
    return rows.map((row) => {
...
function normalizeCSV(inputFilePath) {
  ...
  let normalizedCSV =
    "Nome;Valor;Último Dividendo;Dividend Yield;Valor Patrimonial Por Cota;P/VP;Liquidez Diária;Valor em Caixa;DY CARG;Valor CARG;Patrimônio;Número de Cotistas;Tipo de Gestão;Número de Cotas\n";
...
async function scrapeWebsite(url) {
  ...
  const strongElement = await page.$$eval(
    ".top-info .info strong",
    (elements) => {
...
function normalizeData(data) {
  ...
  data.forEach((fundo) => {
    ...
    if (fundo.valorization !== undefined) {
      fundo.valorization = parseFloat(
        fundo.valorization.replace("%", "").replace(",", ".")
      );
    }
...
function customSort(a, b) {
  ...
  const pontuacaoA =
    a.pvp * pesoPvp +
    a.dividendYield * pesoDividendYield +
    a.valorization * pesoValorizacao +
    a.lucro * pesoLucro +
    a.actualValue * pesoValorAtual;
  const pontuacaoB =
    b.pvp * pesoPvp +
    b.dividendYield * pesoDividendYield +
    b.valorization * pesoValorizacao +
    b.lucro * pesoLucro +
    b.actualValue * pesoValorAtual;
